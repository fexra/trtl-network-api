    ...
    bounds = L.latLngBounds(southWest, northEast), mymap.setMaxBounds(bounds), d3.json("./data/world_map.json", function(a, b) {
        ...
        mymap.on("zoomend", g), setInterval(function() {
            ...
            if (f.length < 6 && Math.random() < .2) {
                ...
                console.log(a + " " + b), f[f.length - 1].setPlaneColor(node_color), f[f.length - 1].setRoadColor(node_color), f[f.length - 1].setBeginColor(node_color), f[f.length - 1].setEndColor(node_color), f[f.length - 1].init({
